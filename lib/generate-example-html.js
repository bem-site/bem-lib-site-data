...
function getBemjson(examplePath) {
    ...
    try {
        bemjson = fs.readFileSync(examplePath + '.short.bemjson.js', 'utf8');
    } catch(e) {
        bemjson = fs.readFileSync(examplePath + '.bemjson.js', 'utf8');
    }

    return nodeEval(bemjson);
}
...
function getDeps(examplePath, exampleName) {
    var exampleDir = path.dirname(path.dirname(examplePath)),
        blockName = path.basename(exampleDir).split('__')[0],
        blockDocsDir = exampleDir.replace(/\/(.*?)\.examples\//, '/$1.docs/').split('__')[0],
        dataPath = path.join(blockDocsDir, blockName + '.data.json'),
        data = require(dataPath),
        examples = data.en.examples.concat(data.ru.examples).concat(data.uk.examples), // FIXME: упячка
        deps = [];
